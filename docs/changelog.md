...
- **M7 (AI Auto-Registration)** — `POST /api/v1/items/auto-register` plus a matching `auto_register_item` MCP tool register items end-to-end from a product image: ISBN/JAN lookup → iterative AI vision (max 3 calls, schema-subset retries) → category resolution → direct `INSERT INTO item`. Gated by the operator-managed `ai.auto_register` flag (default off) with silent fallback to the legacy draft-ready flow. New Web UI checkbox "AI自動登録モード" on the image-upload screen. Standalone OpenAPI fragment for SDK generators at `config/openapi-diff-ai-auto-register.yaml`. See the [integration guide](integrations/ai-auto-register.md).
