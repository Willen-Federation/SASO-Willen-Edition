...
final class ChangeStatusDIContainer implements DIContainer
{
    use Flow;
    ...
    {
        $this->ctrl = new ItemController($query, new ChangeStatusController($post, $now));
        if(empty($post)) {
            $this->usecase = new ItemUsecase(
                new DbFinder(),
                new ItemPresenter(
                    new ChangeStatusView(),
                    new common\FailView(),
                ),
            );
        } else {
            $this->usecase = new ChangeStatusUsecase(
                new DbFinder(),
                new DbUpdater(),
                new DbTransaction(),
